...
from ..utils import ConvModule, Scale, bias_init_with_prob, cls_color
import numpy as np
INF = 1e8
import time
import pdb
...
class SoloHead(nn.Module):
	...
	# def init_weights(self):
	# 	bias_cls = bias_init_with_prob(0.01)
	# 	bias_msk = bias_init_with_prob(0.01)


	# 	for m in self.cls_convs:
	# 		kaiming_init(m.conv,bias=bias_cls)
	# 	for m in self.mask_convs:
	# 		kaiming_init(m.conv,bias=bias_msk)
	   	
	# 	for m in self.solo_cls:
	# 		kaiming_init(m, bias=bias_cls)
	# 	for m in self.solo_mask:
	# 		kaiming_init(m, bias=bias_msk)
	...
	def loss(self,
		...
		loss_mask = self.dict_weight * (loss_mask/num_imgs)
